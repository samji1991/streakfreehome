...
class WPPP_Front_End {
	...
	public function products_per_page_dropdown() {
		...
		?><form method="<?php echo esc_attr( $method ); ?>" action="<?php echo esc_url( $action ); ?>" class="form-wppp-select products-per-page" style='float: right; margin-left: 5px;' class="form-wppp-select products-per-page"><?php
			...
			foreach( $products_per_page_options as $key => $value ) :
				...
				$ppp_text = apply_filters( 'wppp_ppp_text', __( '%s products per page', 'allstore-custom-types' ), $value );
				esc_html( printf( $ppp_text, $value == -1 ? __( 'All', 'allstore-custom-types' ) : $value ) ); // Set to 'All' when value is -1
				?></option><?php
