...
class lessc {
	...
	public function compileFile($fname, $outFname = null) {
		...
		if ($outFname !== null) {
			...
			$wp_filesystem->put_contents($outFname, $out);
...
class lessc_formatter_classic {
	...
	public function block($block) {
		...
		if (!empty($block->selectors)) {
			...
			echo strip_tags($pre.implode($selectorSeparator, $block->selectors));
			if ($isSingle) {
				echo strip_tags($this->openSingle);
				$inner = "";
			} else {
				echo strip_tags($this->open . $this->break);
		...
		if (!empty($block->lines)) {
			...
			echo strip_tags($inner . implode($glue, $block->lines));
			if (!$isSingle && !empty($block->children)) {
				echo strip_tags($this->break);
			}
		}
		...
		if (!empty($block->selectors)) {
			if (!$isSingle && empty($block->children)) echo strip_tags($this->break);

			if ($isSingle) {
				echo strip_tags($this->closeSingle . $this->break);
			} else {
				echo strip_tags($pre . $this->close . $this->break);
			}
