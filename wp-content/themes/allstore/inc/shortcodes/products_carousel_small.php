<?php
add_action( 'vc_before_init', 'allstore_products_carousel_small_integrate_vc' );
function allstore_products_carousel_small_integrate_vc () {
    vc_map( array(
        'name' => esc_html__( 'Products Carousel Small', 'allstore' ),
        'base' => 'allstore_products_carousel_small',
...
add_filter( 'vc_autocomplete_allstore_products_carousel_small_ids_callback', 'allstore_product_carousel_smallIdAutocompleteSuggester', 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_allstore_products_carousel_small_ids_render', 'allstore_product_carousel_smallIdAutocompleteRender', 10, 1 ); // Render exact product. Must return an array (label,value)
//For param: ID default value filter
add_filter( 'vc_form_fields_render_field_allstore_products_carousel_small_ids_param_value', 'allstore_products_carousel_smallIdsDefaultValue', 10, 4 ); // Defines default value for param if not provided. Takes from other param value.
 ...
 */
function allstore_product_carousel_smallIdAutocompleteSuggester( $query ) {
    global $wpdb;
 ...
 * @return bool|array
 */
function allstore_product_carousel_smallIdAutocompleteRender( $query ) {
 ...
 * @return string
 */
function allstore_products_carousel_smallIdsDefaultValue( $current_value, $param_settings, $map_settings, $atts ) {
    ...
    if ( strlen( trim( $value ) ) === 0 && isset( $atts['skus'] ) && strlen( $atts['skus'] ) > 0 ) {
        ...
        foreach ( $skus_array as $sku ) {
            $id = $this->product_carousel_smallIdDefaultValueFromSkuToId( trim( $sku ) );
            if ( is_numeric( $id ) ) {
                $data[] = $id;
            }
        }
...
class WPBakeryShortCode_allstore_products_carousel_small extends WPBakeryShortCode {
    protected function content( $atts, $content = null ) {
        ...
        <?php
        include( trailingslashit( get_template_directory() ) . 'inc/shortcodes/products_carousel_small-content.php' );
        ?>
