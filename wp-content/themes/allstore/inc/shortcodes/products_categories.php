<?php
add_action( 'vc_before_init', 'allstore_product_categories_integrate_vc' );
function allstore_product_categories_integrate_vc () {
    vc_map( array(
        'name' => esc_html__( 'Products categories', 'allstore' ),
        'base' => 'allstore_product_categories',
        'icon' => get_template_directory_uri() . "/img/vc_allstore.png",
        'category' => esc_html__( 'AllStore', 'allstore' ),
        'description' => esc_html__( 'Display products categories loop', 'allstore' ),
        'params' => array(
            array(
                'type' => 'autocomplete',
                'heading' => esc_html__( 'Products Categories', 'allstore' ),
                'param_name' => 'ids',
                'settings' => array(
                    'multiple' => true,
                    'sortable' => true,
                ...
                'save_always' => true,
                'description' => esc_html__( 'To choose custom ordering please set Order by to the Custom OR you can also leave empty this field and show product categories automatically by Order by field', 'allstore' ),
            ),
            array(
                'type' => 'textfield',
...
add_filter( 'vc_autocomplete_allstore_product_categories_ids_callback', 'allstore_productCategoryAutocompleteSuggester', 10, 1 ); // Get suggestion(find). Must return an array
add_filter( 'vc_autocomplete_allstore_product_categories_ids_render', 'allstore_productCategoryRenderByIdExact', 10, 1 ); // Render exact category by id. Must return an array (label,value)


function allstore_productCategoryAutocompleteSuggester( $query, $slug = false ) {
    global $wpdb;
    $cat_id = (int) $query;
    $query = trim( $query );
...
function allstore_productCategoryRenderByIdExact( $query ) {
    $query = $query['value'];
    $cat_id = (int) $query;
    $term = get_term( $cat_id, 'product_cat' );

    return allstore_productCategoryTermOutput( $term );
}


function allstore_productCategoryTermOutput( $term ) {
    $term_slug = $term->slug;
...
class WPBakeryShortCode_allstore_product_categories extends WPBakeryShortCode {
            ...
            <div class="flexslider brands-list<?php echo esc_attr( $css_class ); ?>">
                <ul class="slides">
                    <?php foreach ($product_categories as $category) :
                        ?>
                        <li class="brands-i">
                            <a class="brands-i-link" href="<?php echo get_term_link($category->term_id); ?>">
								<span class="brands-i-img">
                                    <?php woocommerce_subcategory_thumbnail( $category ); ?>
								</span>
                                <?php if ($name_show !== 'hide') : ?>
                                <p class="brands-i-ttl"><?php echo esc_attr($category->name); ?> <span class="brands-i-count">(<?php echo esc_attr($category->count); ?>)</span></p>
                                <?php endif; ?>
                            </a>
                        </li>
                    <?php endforeach; ?>
